...
//					}
					...
					if(globalCanId == stdid ){
						switch (pData[0]){
							case 0x00:
								// tod mb:: das ist noch scheiße. unbedingt nummer aus enum
...
// todo mb: beschreiben für eigenimpulse
...
//		p[7] = checkSum;
		...
		uint32_t canId = PULSE_OPENCAN_OFFSET + channel + GetGlobalCanNodeId();
		SendCan(canId, p, 8);
