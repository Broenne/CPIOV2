...
void Init_TimerForPulsTime(void) {
	...
	pulseTimerInstance.Init.Prescaler = 36;
	...
	HAL_NVIC_SetPriority(TIM3_IRQn, 6, 5); // extrem hohe priorität
...
void SendPulsePerCanTask(void * pvParameters) {
	...
	while (1) {
		...
		if (xQueueReceiveFromISR(PulsQueue, &currentMessage, 10) == pdTRUE) {
